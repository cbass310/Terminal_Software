...
            }
            ...
            // Math variance applied linearly for head-to-head calculations
...
        } else {
            // THE EXPONENTIAL FIX: Quadratic scaling to punish low-AV teams
            const maxExpectedScore = 19.5; 
            let winRatio = playerWeightedScore / maxExpectedScore;
            if (winRatio > 1) winRatio = 1;

            let winPercentage = Math.pow(winRatio, 2);
            finalWins = Math.round(winPercentage * 17);
            
            // "Any Given Sunday" RNG Variance
...
        }
        ...
        // AUTH CHECK AND FUNNEL LOGIC
        if (activeMode !== 'duel') {
            const isLoggedIn = await pushToLeaderboard(finalWins, finalLosses, playerWeightedScore);
            
            if (!isLoggedIn) {
                // Guest-to-Premium Funnel
                messageHtml += `
                    <div class="mt-8 border border-neon/50 bg-neon/10 p-6 rounded-xl text-center shadow-[0_0_15px_rgba(57,255,20,0.1)]">
                        <h4 class="text-white font-black uppercase tracking-widest mb-2">GUEST ACCOUNT DETECTED</h4>
                        <p class="text-slate-400 text-sm font-mono mb-6">Your score of ${finalWins}-${finalLosses} is untracked.</p>
                        <button onclick="window.location.href='/login.html'" class="w-full bg-neon text-black font-black px-6 py-4 rounded-xl uppercase tracking-widest hover:bg-green-400 transition-all shadow-[0_0_20px_rgba(57,255,20,0.5)]">
                            INITIALIZE OPERATOR ACCOUNT TO SAVE SCORE
                        </button>
                    </div>
                `;
            } else {
                // Verified Push UI Update
                messageHtml += `
...
    if (typeof window.db === 'undefined') {
        console.warn("⚠️ pushToLeaderboard: window.db is undefined. Operating in Guest Mode.");
        return false;
    }
...
        const { data: { session }, error: authError } = await window.db.auth.getSession();
        ...
        if (authError) {
            console.error("🚨 pushToLeaderboard Auth Error:", authError);
            return false;
        }
...
            
            if (dbError) {
                ...
                return true; // Return true because they are logged in, just a DB hitch.
            } else {
