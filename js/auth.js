    ...
    try {
        ...
        if (session) {
            ...
            if(authNavDesktop) authNavDesktop.innerHTML = `
                ...
                <button onclick="db.auth.signOut(); window.location.replace('login.html');" class="text-xs font-bold bg-white/10 hover:bg-red-500/20 hover:text-red-400 text-slate-400 border border-white/10 hover:border-red-500/50 py-2 px-4 rounded-lg transition-all uppercase tracking-widest ml-4">Sign Out</button>
            `;
            if(authNavMobile) authNavMobile.innerHTML = `
                ...
                <button onclick="db.auth.signOut(); window.location.replace('login.html');" class="block w-full mt-2 px-4 py-3 rounded-xl text-base font-black text-red-400 bg-red-500/10 border border-red-500/30 uppercase tracking-wide transition text-center">Sign Out</button>
            `;
        } else {
            // USER IS LOGGED OUT
            if(authNavDesktop) authNavDesktop.innerHTML = `
